	...
	public struct AwaitInstructionAwaiter<T> : IAwaitInstructionAwaiter, INotifyCompletion where T : IAwaitInstruction
	{
		...
		public bool Evaluate()
		{
			try
			{
				...
				if(!owner || !owner.gameObject.activeSelf || !owner.enabled)
					return true;
		...
		public AwaitInstructionAwaiter(in T inst, MonoBehaviour owner, FrameScheduler scheduler)
		{
			instruction = inst;
			continuation = null;
			this.owner = owner;
		...
		public void OnCompleted(Action continuation)
		{
			...
			AsyncManager.Instance.AddContinuation(this);
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public AwaitInstructionAwaiter<T> ConfigureAwait(MonoBehaviour owner)
		{
			this.owner = owner;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public AwaitInstructionAwaiter<T> ConfigureAwait(MonoBehaviour owner, FrameScheduler scheduler)
		{
			this.owner = owner;
			Scheduler = scheduler;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public AwaitInstructionAwaiter<T> ConfigureAwait(MonoBehaviour owner, CancellationToken cancellationToken, FrameScheduler scheduler)
		{
			this.owner = owner;
			this.cancellationToken = cancellationToken;
			Scheduler = scheduler;
